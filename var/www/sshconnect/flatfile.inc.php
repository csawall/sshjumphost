...
if (isset($_POST['submit'])) {
  ...
  $f = fopen($data_file,'w+');
  // added by sawall 01/03/10
  // create array so that DB can be sorted 
  $db_array = array();
  if ($f) {
    ...
    for( $i=0; $i < count($_POST[$structure[0]['name']]); $i++ ) {
      ...
      #fputs($f,$s."\n");
      // added by sawall 01/03/10
      // push each line onto the end of the array
      array_push($db_array, $s);
      
    } // for
    // added by sawall 01/03/10
    // sort array and then write each line to file, sorted in alphabetical order
    sort($db_array);
    foreach ($db_array as $db_file_line)
	{
	   fputs($f,$db_file_line."\n");
	} // foreach
    fclose($f);
  } // if
}
...
$sshcatname = explode("/", $data_file);
#echo "<body><h1>$data_file</h1>";
echo "<body><h1>editing: $sshcatname[1]</h1>";
